	...
	public boolean onTouchEvent(MotionEvent event) {
		if(event.getAction() == MotionEvent.ACTION_DOWN) {
			for(int i=0; i<list.size(); i++) {
				...
				if(contents.rect.contains((int)event.getX(), (int)event.getY())) {
					//Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse("geo:" + contents.latitude + "," + contents.longitude + "?z=16"));
					//ContentsView.this.getContext().startActivity(intent);
					
					Intent intent = new Intent(ContentsView.this.getContext(), LocationOverlayActivity.class);
